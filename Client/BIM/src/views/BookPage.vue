<template>
  <div
    class="container overflow-x-auto shadow-md sm:rounded-lg px-10 mx-auto max-w-7xl mt-20"
  >
    <div class="pb-4 bg-white dark:bg-gray-900">
      <h1 class="text-2xl mb-4 font-bold py-7">Books Details</h1>
      <div class="flex items-center justify-between">
        ...
        <div class="relative mt-1 flex-grow">
          ...
          <input
            ...
            id="table-search"
            v-model="searchQuery"
            class="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-full sm:w-80 bg-gray-50 focus:ring-red-500 focus:border-red-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        ...
        >
          ...
          >
          <span
            class="absolute right-0 h-full w-10 rounded-lg bg--500 flex items-center justify-center transform group-hover:translate-x-0 group-hover:w-full transition-all duration-300"
          >
            <svg
    ...
    >
      ...
      <tbody>
        ...
        >
          ...
          <td class="px-6 py-4">{{ book.publishedDate }}</td>
  ...
  <div
    ...
    tabindex="-1"
    aria-hidden="true"
    class="fixed inset-0 z-50 flex justify-center items-center w-full h-full backdrop-blur-lg bg-gray-800 bg-opacity-50"
  >
    <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <!-- Add New Book Form -->
        <form @submit.prevent="addBook" class="p-4 md:p-5">
  ...
  >
    <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <!-- Add New Book Form -->
        <form @submit.prevent="updateBook" class="p-4 md:p-5">
          <div class="grid gap-4 mb-4 grid-cols-2">
            <div class="col-span-2">
              <label
                for="title"
  ...
  methods: {
    ...
    toggleUpdateModal(bookId = null) {
      ...
      if (bookId) {
        ...
        this.updateModal = false;
        console.log("Update modal state:", this.updateModal);
      }
    },

    fetchBookDetails(id) {
      console.log("Inside fetchBookDetails with ID:", id); // Debugging log
      ...
      axios
        .get(`https://localhost:7209/api/books/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((response) => {
          this.updatedBook = response.data; // Store book details
          this.bookToUpdateId = id; // Set ID for updating
    ...
    fetchBooks() {
      ...
      if (token) {
        axios
          .get("https://localhost:7209/api/books", {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
          .then((response) => {
            this.books = response.data; // Populate the books array with the response data
          })
          .catch((error) => {
    ...
    // Method to search books based on the query
    searchBooks() {
      if (this.searchQuery.length > 0) {
        ...
        if (token) {
          axios
            .get(
              `https://localhost:7209/api/books/search?keywords=${this.searchQuery}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
            ...
            .then((response) => {
              this.books = response.data; // Update books with the search result
            })
            .catch((error) => {
        ...
        this.fetchBooks(); // If search query is empty, fetch all books
      }
    },
    deleteBook(bookId) {
      const token = localStorage.getItem("jwt_token");
      if (token) {
        axios
          .delete(`https://localhost:7209/api/books/${bookId}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
    ...
    addBook() {
      ...
      if (token) {
        axios
          .post("https://localhost:7209/api/books", this.newBook, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
    ...
    updateBook() {
      ...
      if (!this.bookToUpdateId || Object.keys(this.updatedBook).length === 0) {
        console.error("Book ID or update data missing");
        return;
      }

      axios
        .patch(
          `https://localhost:7209/api/books/${this.bookToUpdateId}`,
          this.updatedBook,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        .then((response) => {
          const index = this.books.findIndex(
            (book) => book.id === this.bookToUpdateId
          );
          if (index !== -1) {
        ...
        .catch((error) => {
          ...
          this.$toast.error("Failed to update the book. Please try again.", {
            position: "top-right",
            timeout: 3000,
          });
        });
